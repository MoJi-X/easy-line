...
- 范围内：`src/services/scheduler.ts`、`node-cron` 注册逻辑、任务执行日志、调度状态快照与健康检查所需的基础信息。
...
- implementation notes：执行体围绕“获取告警信息”任务语义命名，不再保留天气逻辑；执行记录至少保存 `taskId`、`taskName`、`ownerUserId`、`cron`、`status`、`triggeredBy`、`executedAt`、`durationMs`、`message`；单任务失败不得影响其他任务继续运行。
...
- 风险：执行体若后续接入真实告警查询接口，外部返回结构波动会影响任务结果渲染稳定性。
- 回退：先保留最小告警任务执行记录与日志，确保调度生命周期和注册重载链路可验证，再补真实告警拉取与主动推送适配。

## 当前实现基线
- 当前代码基线要求 Scheduler 依赖任务仓储、显式使用 `node-cron`、支持 `reload` 语义，并把执行目标固定到告警信息获取任务。
- 下一步可在当前生命周期框架上补齐真实告警拉取、LINE Push 和健康检查详情，而不再回退到天气任务模型。
