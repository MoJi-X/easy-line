...
## 统一取舍
- 文档冲突时以《需求规格说明书》为准。
- 环境变量命名采用 `LINE_CHANNEL_SECRET`、`LINE_CHANNEL_ACCESS_TOKEN`、`LLM_API_KEY`、`LLM_BASE_URL`、`LLM_MODEL`、`TAVILY_API_KEY`、`WEATHER_API_KEY`、`PORT`；新增告警与工单配置统一使用 `ALARM_*`、`WORKORDER_*`、`DEFAULT_*` 前缀。
...
- 告警分析与工单创建属于按需触发的 Agent 工作流，不接入天气调度系统，也不引入数据库持久化。
